...
/* lock used for the sleepers list */
...
static bool ready_before(const struct list_elem *a,const struct list_elem *b,void *aux UNUSED)
{
...
{
  if(cur != list_end(&sleep_list) ) // if sleep list is not empty
  {
...
thread_tick (void) 
{
  ...
  thread_wakeup(list_begin(&sleep_list), timer_ticks()); 
  /* Enforce preemption. */
...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered (&ready_list, &t->elem,ready_before, NULL);
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_ordered (&ready_list, &cur->elem,ready_before,NULL);
...
/* Temporarily increases the priority */
void thread_priority_temporarily_up()
{

  struct thread *cur_thread = thread_current();  

  //save the priority of current thread
  cur_thread->prev_priority = cur_thread->priority;    
...
{
  struct thread *cur_thread = thread_current();
  cur_thread->priority = cur_thread->prev_priority;
}

/* making the current thread go to sleep and updating it's wakeup time*/
void thread_block_till(int64_t wakeup_at, int cur_time)
{
  // disabling the interrupts
  enum intr_level old_int = intr_disable();
  // lock the list so that no other thread can get access to the sleep list
  lock_acquire (&sleepers_lock); 
  //get the current thread
  struct thread *cur_thread = thread_current();

  if(cur_time > wakeup_at) return;

  //check whether the thread is currently running or not
  ASSERT(cur_thread->status == THREAD_RUNNING); 

  // setting the wakeup time of the thread.
  cur_thread->wakeup_at = wakeup_at; 

  // insert it to the sleep list, where before is the comparator for sleep_list
  list_insert_ordered(&sleep_list , &( thread_current()->elem ), before, NULL); 
 
 //release the lock of the sleepers list
...
void thread_set_next_wakeup()
{
  ...
  // get the list_elem with least wake up time
  struct list_elem * cur = list_begin( &sleep_list );

  // sleep list is not empty
  if(cur != list_end( &sleep_list )) 
  {
      struct thread *top_thread = list_entry(cur,struct thread,elem); 
      int64_t start = timer_ticks();
      if(top_thread->wakeup_at <= start)
      {
         list_remove(cur);
